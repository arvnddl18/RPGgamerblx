...
local EquipmentSlots = require(ReplicatedStorage.Shared.Config.EquipmentSlots)
...
function EquipmentUI.new(playerGui)
	...
	panel.Position = UDim2.new(1, -216, 0.5, -170)
...
function EquipmentUI:Update(equipped, itemsConfig)
	for slot, row in self._slots do
		...
		if itemId and itemsConfig[itemId] then
			local label = EquipmentSlots.LABELS[slot] .. ": " .. itemsConfig[itemId].name
